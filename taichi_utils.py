...
@ti.func
def interp_grad_center_2(vf, p, dx, BL_x=0.5, BL_y=0.5, is_y=False):
    u_dim, v_dim = vf.shape
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            ...
            interped += value * N_2(x_p_x_i) * N_2(y_p_y_i)
            dpos = ti.Vector([x_p_x_i, y_p_y_i])
            new_C += 4 * N_2(x_p_x_i) * N_2(y_p_y_i) * value.outer_product(dpos)
    return interped, ti.Vector([partial_x, partial_y]), new_C


@ti.func
def interp_grad_center(vf, p, dx, BL_x=0.5, BL_y=0.5, is_y=False):
    u_dim, v_dim = vf.shape
    ...
    partial_x = ti.Vector([0.0, 0.0])
    partial_y = ti.Vector([0.0, 0.0])
    ...
    for i in range(-1, 3):
        for j in range(-1, 3):
            ...
            dpos = ti.Vector([x_p_x_i, y_p_y_i])
            new_C += 4 * (1.0 / dx) * N_2(x_p_x_i) * N_2(y_p_y_i) * value.outer_product(dpos)
...
def interp_u_MAC_grad(u_x, u_y, p, dx):
    u_p, grad_u_p, C = interp_grad_2(u, p, dx, BL_x=0.5, BL_y=0.5, is_y=False)
